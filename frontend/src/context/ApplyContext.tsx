...
import { applications, Application } from "../types/apply";
...
export function ApplyProvider({children} : {children : React.ReactNode}) {
    ...
    useEffect(() => {
        ...
        //console.log("This is saved venues: " + savedVenues);
        ...
        if (!savedApps || JSON.parse(savedApps).length === 0) {
            ...
            //console.log("run else: " + savedVenues);
            setAllApps(JSON.parse((savedApps)));
        }
    ...
    const removeApp = (id : number) => {
        setAllApps(allApplications.filter((app : Application) => app.id !== id));
    }
